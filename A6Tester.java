...
public class A6Tester {
	...
	public static void testInsertionAndRemoval() {
		...
		try{
			...
			displayResults(true, "insert + remove on 100000 element PQ");
		}
		...
		try{
			...
			pq.removeMin();
			displayResults(false, "insert + remove on 100000 element PQ");
...
public static void testPatient() {
	...
	public static void testEmergencyRoom() {
		...
		EmergencyRoom largeroom = new EmergencyRoom();
		try{
			for(int i =0; i<10000;i++){
				largeroom.addPatient(p1);
			}
			displayResults(true, "test add 10000 patients");
		}catch(HeapFullException e){
			displayResults(false, "test add 10000 patients");
		}
		try{
			for(int f =0; f<10000; f++){
				nextP = largeroom.nextPatient();
			}
			System.out.println(nextP);
			nextP = largeroom.nextPatient();
			displayResults(nextP==null, "test nextPatient");
		}catch(HeapEmptyException e){
			displayResults(false, "test nextPatient");
		}
